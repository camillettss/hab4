...
class MainServer(asyncore.dispatcher):
  ...
  def handle_accept(self):
    ...
    self.eng.outgoing.append(conn)
    playerid = random.randint(1000, 1000000)
    playerminion = Player(columns,rows,self.eng)
    self.eng.robots[playerid] = playerminion
...
class Engine():
    ...
    def updateWorld(self, data):
        print(data) # invia i dati ai client connessi
    
    def online_start(self):
        '''start a local server'''
        self.robots={}
        MainServer(8080, self)
        asyncore.loop()
        print('out')
